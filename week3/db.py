"""
Week 3 – Database layer (SQLite for now; switch to Postgres on Railway later).
"""
import os
import sqlite3
...
# SQLite file in week3 folder. For Railway later you can switch to Postgres (DATABASE_URL).
...
def init_db():
    ...
    with get_conn() as conn:
        ...
        """)
        # Default restaurant if none
        cur = conn.execute("SELECT 1 FROM restaurants LIMIT 1")
        if cur.fetchone() is None:
            conn.execute(
                "INSERT INTO restaurants (name, wati_phone) VALUES (?, ?)",
                ("Demo Restaurant", None),
            )
            conn.execute(
...
def get_or_create_conversation(restaurant_id: int, customer_phone: str) -> int:
    """Return conversation_id (create if needed)."""
    with get_conn() as conn:
        cur = conn.execute(
